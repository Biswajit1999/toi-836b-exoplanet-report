"""Transit-window analysis of a real TESS SPOC light curve for TOI-836 b.
...
DATA_FILE = DATA_DIR / "tess2019112060037-s0011-0000000440887364-0143-s_lc.fits"
PLANET = "TOI-836 b"
PERIOD_DAYS = 3.81673
EPOCH_BJD = 2458599.9953
DURATION_HOURS = 1.805
SECTOR = 11
FIGURE_FILE = FIG_DIR / "toi836b_tess_transit.png"
...
def main() -> dict[str, float]:
    ...
    ax.scatter(hours, (result["flux"] - 1) * 1e6, s=5, alpha=0.12, color="#6741d9", label="quality-filtered cadences")
    ...
    ax.plot(hours[order], (result["model"][order] - 1) * 1e6,
            color="#6741d9", linewidth=2.2, label="fitted box model")
    ax.axvspan(-DURATION_HOURS / 2.0, DURATION_HOURS / 2.0, color="#6741d9", alpha=0.08)
